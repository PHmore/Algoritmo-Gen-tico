...
for geracao in range(geracoes):
    ...
    event, values = Carregamento.read(timeout=0.00001)  # Adiciona timeout para evitar bloqueio
    Carregamento.refresh()
    if event == sg.WINDOW_CLOSED:
        janela_parametros.close()
        break

    populacao = sorted(populacao, key=lambda x: fitness(x), reverse=1)
    nova_populacao = []
...
tela_resultados = [
    ...
    #,[sg.Listbox(values=log, size=(30, len(log)), key='-LISTBOX-')],
]
